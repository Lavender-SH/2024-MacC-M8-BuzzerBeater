...
- 애플워치에서 나침반을 구현한 방법</br>
- 실시간 바람의 방향을 보여주는 기능</br>
- 바람의 방향에 맞게 최적의 돛의 각도를 가이딩하는 기능</br>
- 애플워치와 아이폰의 데이터 연동</br>
- 지도에서 항해 경로를 표시하는 기능</br>
- 요트에 하드웨어를 부착하여 블루투스로 실제 돛의 각도를 보여주는 기능</br>

</br>

### 애플워치에서 나침반을 구현한 방법
- WindTalker앱에서 나침반(Compass)은 사용자 경험을 극대화하기 위해 SwiftUI와 CoreLocation을 활용하여 구현되었습니다. 아래는 나침반의 주요 기능과 구현에 대한 간략한 설명입니다.</br>

 1. CoreLocation
 - CLLocationManager를 사용해 사용자의 실시간 위치와 방향 정보를 수집.</br>
 - trueHeading 값을 활용하여 현재 나침반 방향을 동적으로 업데이트.</br>
 2. GeometryReader
 - GeometryReader를 사용해 디스플레이 크기에 맞게 동적으로 크기를 조정합니다.</br>
...
- 각도 angle에 따라 원 위의 특정 점 (x, y)를 계산합니다
x = r * cos(각도) // cos(각도)는 가로 방향(x축)의 비율
y = r * sin(각도) // sin(각도)는 세로 방향(y축)의 비율
이 식은 삼각 함수를 활용한 건데, 쉽게 풀어보면 아래와 같습니다:
